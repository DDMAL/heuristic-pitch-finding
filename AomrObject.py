    ...
    def interpolate_staff_locations(self):
        ...
        for i, staff in enumerate(self.staff_locations):
            ...
            for j, line in enumerate(staff['line_positions']):  # for each line
                ...
                for k, pt in enumerate(line):                   # for each point tuple

                    if self.close_enough(pt[0], refLine[k+nudge][0]):  # if same x as ref
                        # print "append", pt
                        newLine.append(pt)                      # no interpolation necessary
                    else:

                        for l in range(k+1, len(refLine)):
                            nudge += 1
                            
                            if self.close_enough(pt[0], refLine[l][0]): # if next ref point has the same x value 
                                # interpolate points between k and l

                                for m in range(l-k):
                                    calc = line[k-1][1] + (pt[1] - line[k-1][1]) * ( (m + 1) / (l - k) ) # this does not seem to do what I think it does, but it still works well enough so :/

                                    if(not self.close_enough(refLine[k+m][0], newLine[len(newLine)-1][0])):
                                        # print "append*", (refLine[k+m][0], calc)
                                        newLine.append((refLine[k+m][0], calc))    # k.val + (l.val-k.val * (m+1)/(l-k) )
